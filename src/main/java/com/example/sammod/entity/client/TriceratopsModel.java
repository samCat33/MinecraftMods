...
// Paste this class into your mod and generate all required imports
    ...
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(SamMod.MOD_ID, "triceratops"), "main");
    ...
    @Override
    public void setupAnim(TriceratopsEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float netHeadPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        this.applyHeadRotation(netHeadYaw, netHeadPitch);


        //The last parameter of animateWalk() controls how dramatically the limbs move
        this.animateWalk(TriceratopsAnimations.TRIKE_WALKING, limbSwing, limbSwingAmount, 4f, 6.5f);
    ...
    private void applyHeadRotation(float netHeadYaw, float netHeadPitch){
        netHeadYaw = Mth.clamp(netHeadYaw, -60.0F, 60.0F);
        netHeadPitch = Mth.clamp(netHeadPitch, -60.0F, 60.0F);

        this.head.yRot = netHeadYaw * (float) Math.PI / 180.0F;
        this.head.xRot = netHeadPitch * ((float) Math.PI / 180.0F);
    }
