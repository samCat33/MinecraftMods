...
public class HammerItem extends DiggerItem {
    ...
    public static List<BlockPos> getBlocksToBeDestroyed(int range, BlockPos initialPos, ServerPlayer player){
        ...
        if (traceResult.getDirection() == Direction.DOWN || traceResult.getDirection() == Direction.UP){
            for (int x = -range; x <= range; x++){
                for (int y = -range; y <= range; ++y){
                    positions.add(new BlockPos(initialPos.getX() + x, initialPos.getY(), initialPos.getZ() + y));
                }
            }
        }
        ...
        if (traceResult.getDirection() == Direction.EAST || traceResult.getDirection() == Direction.WEST){
            for (int x = -range; x <= range; x++){
                for (int y = -range; y <= range; ++y){
                    positions.add(new BlockPos(initialPos.getX(), initialPos.getY() + y, initialPos.getZ() + x));
                }
            }
        }
